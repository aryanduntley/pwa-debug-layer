...
export const reactFindByTextHandler = async (
  ...
  try {
    ...
    return errorResponse(
      `react_find_by_text failed: ${(err as Error).message}`,
      [
  ...
  if (response.error) {
    return errorResponse(
      `react_find_by_text nmh error: ${response.error.message}`,
      [
  ...
  if (isToolErrorPayload(response.payload)) {
    return errorResponse(
      `react_find_by_text: ${response.payload.error.message}`,
      [
  ...
  if (!isFindByTextSuccess(response.payload)) {
    return errorResponse(
      'react_find_by_text returned a malformed payload (missing matches/truncated/rootCount).',
      [
...
export const reactFindByTextTool: ToolDef<typeof inputSchema> = Object.freeze({
