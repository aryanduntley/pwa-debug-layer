...
export const svelteFindByTextMcpHandler = async (
  ...
  try {
    ...
    return errorResponse(`svelte_find_by_text failed: ${(err as Error).message}`, [
  ...
  if (!isFindSuccess(response.payload)) {
    return errorResponse(
      'svelte_find_by_text returned a malformed payload (missing matches/truncated).',
      ['Check packages/extension/src/svelte/find_by_text.ts.'],
    );
  }
  ...
  if (response.payload.matches.length === 0) {
    nextSteps.push(
      'No matches. Either no component text matched (pattern is new RegExp, no flags, case-sensitive; keep exact:false for substring), OR this is a production build with no __svelte_meta — call svelte_components to check dev:true.',
    );
  }
  if (response.payload.truncated) {
    nextSteps.push(
      'truncated:true — max_matches (default 20) reached. Raise it or tighten the pattern.',
    );
  }
...
export const svelteFindByTextTool: ToolDef<typeof inputSchema> = Object.freeze({
