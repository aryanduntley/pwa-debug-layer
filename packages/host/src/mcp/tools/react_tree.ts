...
type ReactTreeNode = {
  ...
  readonly children: ReadonlyArray<ReactTreeNode>;
};

type ReactTreePayload = {
  readonly roots: ReadonlyArray<ReactTreeNode>;
  readonly truncated: boolean;
  readonly rootCount: number;
};

const isTreeNode = (v: unknown): v is ReactTreeNode => {
...
const readPayload = (raw: unknown): ReactTreePayload | null => {
...
export const reactTreeHandler = async (
  ...
  try {
    ...
    return errorResponse(`react_tree failed: ${(err as Error).message}`, [
  ...
  if (payload === null) {
    return errorResponse(
      'react_tree returned a malformed payload (missing roots/truncated/rootCount).',
      [
  ...
  if (payload.rootCount === 0) {
    nextSteps.push(
      'rootCount===0 means no React roots detected on the page. Verify the page actually renders React (use evaluate with __REACT_DEVTOOLS_GLOBAL_HOOK__) or that ReactDOM.createRoot has run before this call.',
    );
  }
...
export const reactTreeTool: ToolDef<typeof inputSchema> = Object.freeze({
  name: 'react_tree',
  description:
